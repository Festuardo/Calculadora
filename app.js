...
$('quoteForm').addEventListener('submit',e=>{e.preventDefault();try{
 const q=calculateQuote(Number($('amount').value),Number($('term').value),Number($('rate').value),$('type').value);currentQuote=q;
 const metric=(label,value)=>`<div class="metric"><span>${label}</span><strong>${formatCurrency(value)}</strong></div>`;
 $('table-container').innerHTML=`<h2>Tu cotización</h2><p class="quote-meta">Monto: ${formatCurrency(q.amount)} · Plazo: ${q.term} meses · Tasa anual: ${q.rate}%<br>${q.type==='nivelada'?'Cuotas niveladas':'Cuotas sobre saldo'} · Fecha: ${new Date().toLocaleDateString('es-GT')}</p><div class="summary">${metric(q.type==='nivelada'?'Cuota mensual':'Primera cuota',q.rows[0].installment)}${metric('Intereses totales',q.totalInterest)}${metric('Total a pagar',q.total)}</div><div class="table-scroll"><table><thead><tr><th scope="col">Mes</th><th scope="col">Cuota</th><th scope="col">Interés</th><th scope="col">Capital</th><th scope="col">Saldo</th></tr></thead><tbody>${q.rows.map(r=>`<tr><td>${r.month}</td><td>${formatCurrency(r.installment)}</td><td>${formatCurrency(r.interest)}</td><td>${formatCurrency(r.principal)}</td><td>${formatCurrency(r.balance)}</td></tr>`).join('')}</tbody><tfoot><tr><td>Total</td><td>${formatCurrency(q.total)}</td><td>${formatCurrency(q.totalInterest)}</td><td>${formatCurrency(q.amount)}</td><td>${formatCurrency(0)}</td></tr></tfoot></table></div><p class="note">Estimación de capital e intereses. No incluye seguros, comisiones ni otros cargos. Sujeta a las condiciones del crédito. Las cifras mostradas se redondean a dos decimales.</p><p class="print-footer">© ${new Date().getFullYear()} Fgarcia · Todos los derechos reservados.</p>`;
